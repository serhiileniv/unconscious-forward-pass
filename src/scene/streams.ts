...
import { layerZ, tokenX } from './lattice'
 ...
 */
export function streamPoint(trace: StepTrace, l: number, pos: number, nLayers: number, out: THREE.Vector3): THREE.Vector3 {
  const T = trace.ids.length
  if (l < 0) return out.set(tokenX(pos, T), 0, layerZ(0, nLayers) - 2.6)
  const p = trace.layers[l].proj
  return out.set(
    tokenX(pos, T) + p[pos * 3] * WANDER,
    p[pos * 3 + 1] * WANDER,
    layerZ(l, nLayers) + p[pos * 3 + 2] * 0.16,
  )
}
  ...
  setTrace(trace: StepTrace, nLayers: number): void {
    ...
    for (let pos = 0; pos < this.T; pos++) {
      for (let l = -1; l < nLayers - 1; l++) {
        streamPoint(trace, l, pos, nLayers, a)
        streamPoint(trace, l + 1, pos, nLayers, b)
  ...
  update(trace: StepTrace, layerF: number): void {
    ...
    for (let pos = 0; pos < this.T; pos++) {
      for (let l = -1; l < this.nLayers - 1; l++) {
        ...
        const write = l >= 0 ? Math.min(1, trace.layers[l].writeNorm[pos] * 0.22) : 0.35
