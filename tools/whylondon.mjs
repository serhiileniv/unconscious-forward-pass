...
// src/model/gpt2.ts
function vecmat(x, xOff, w, b, n, p2, out, outOff = 0) {
  if (b) out.set(b.subarray(0, p2), outOff);
  else out.fill(0, outOff, outOff + p2);
  for (let k = 0; k < n; k++) {
    const xv = x[xOff + k];
    if (xv === 0) continue;
    const wOff = k * p2;
    for (let j = 0; j < p2; j++) out[outOff + j] += xv * w[wOff + j];
  }
}
...
function dist(prompt) {
  ...
  const p2 = new Float32Array(logits.length);
  p2.set(logits);
  softmax(p2);
  const order2 = Array.from(p2.keys()).sort((a, b) => p2[b] - p2[a]);
  let H = 0;
  for (const v of p2) if (v > 0) H -= v * Math.log2(v);
  return { p: p2, order: order2, H, ids };
}
for (const prompt of [
  ...
  const { p: p2, order: order2, H, ids } = dist(prompt);
  console.log(`
${JSON.stringify(prompt)}`);
  console.log(`  ${ids.length} tokens, entropy ${H.toFixed(2)} bits`);
  console.log(`  top: ${order2.slice(0, 6).map((i) => `${JSON.stringify(m.tok.piece(i))} ${(p2[i] * 100).toFixed(1)}%`).join("  ")}`);
}
var { p, order } = dist("The Eiffel Tower is located in the city of");
var paris = m.tok.encode(" Paris")[0];
var top40 = order.slice(0, 40);
console.log(`
greedy answer: ${JSON.stringify(m.tok.piece(order[0]))}
`);
console.log('temp   P(picks " Paris")   P(picks a wrong city)');
for (const T of [1, 0.8, 0.5, 0.3, 0.2, 0.1]) {
