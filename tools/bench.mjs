...
var GPT2State = class {
  ...
  constructor(model2, maxT) {
    this.model = model2;
    this.maxT = maxT;
    const { nLayer, nEmbd, nHead } = model2.cfg;
  ...
  push(tokenId) {
    ...
    for (let i = 0; i < nEmbd; i++) x[i] = m.wte[tokenId * nEmbd + i] + m.wpe[pos2 * nEmbd + i];
    ...
    const logits2 = new Float32Array(m.cfg.vocabSize);
    for (let v = 0; v < m.cfg.vocabSize; v++) logits2[v] = dot(m.wte, v * nEmbd, h, 0, nEmbd);
    this.t++;
    return logits2;
  }
  /**
  ...
  lensAt(layer, pos2) {
    ...
    const h = new Float32Array(nEmbd);
    layerNormAffine(this.hidden[layer], pos2 * nEmbd, nEmbd, m.lnFg, m.lnFb, eps, h, 0);
    ...
    for (let i = 0; i < nLens; i++) out2[base + i] = (out2[base + i] - mean) / sd;
  }
};

// tools/bench.ts
var DIR = "public/model/gpt2";
var read = (f) => readFileSync(`${DIR}/${f}`);
...
var prompt = process.argv[2] ?? "When Mary and John went to the store, John gave a drink to";
...
var t1 = Date.now();
var logits = new Float32Array();
for (const id of ids) logits = state.push(id);
var prefill = Date.now() - t1;
var out = [];
var t2 = Date.now();
for (let n = 0; n < 8; n++) {
  let best = 0;
